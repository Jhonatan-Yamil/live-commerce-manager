...
def list_intakes(db: Session, status: VoucherMatchStatus | None = None, skip: int = 0, limit: int = 100, user_id: int | None = None):
    ...
    if user_id is not None:
        q = q.filter(VoucherIntake.user_id == user_id)
    if status:
        q = q.filter(VoucherIntake.match_status == status)
    return q.order_by(VoucherIntake.id.desc()).offset(skip).limit(limit).all()


def get_by_id(db: Session, intake_id: int, user_id: int | None = None):
    q = db.query(VoucherIntake).filter(VoucherIntake.id == intake_id)
    if user_id is not None:
        q = q.filter(VoucherIntake.user_id == user_id)
    return q.first()
...
def get_recent_by_hash(db: Session, *, file_sha256: str, since: datetime, user_id: int | None = None):
    q = db.query(VoucherIntake).filter(
            VoucherIntake.file_sha256 == file_sha256,
            VoucherIntake.created_at >= since,
        )
    if user_id is not None:
        q = q.filter(VoucherIntake.user_id == user_id)
    return q.order_by(VoucherIntake.id.desc()).first()

def get_by_external_message_id(db: Session, message_id: str, user_id: int | None = None):
    from app.models.voucher_intake import VoucherIntake
    q = (
        db.query(VoucherIntake)
        .filter(VoucherIntake.external_message_id == message_id)
    )
    if user_id is not None:
        q = q.filter(VoucherIntake.user_id == user_id)
    return q.first()